...
if [ $CURRENT_VER == $PREVIOUS_VER ]; then
    ...
    echo "Certificates have been updated; Copying to new location"
        ...
        openssl pkcs12 -export -in $DESTINATION_PATH/cert.pem -inkey $DESTINATION_PATH/privkey.pem -out $DESTINATION_PATH/unifi.p12 -name unifi -CAfile $DE                     STINATION_PATH/fullchain.pem -caname root -password pass:aircontrolenterprise

    keytool -importkeystore -deststorepass aircontrolenterprise -destkeypass aircontrolenterprise -destkeystore $DESTINATION_PATH/keystore -srckeystore $DE                     STINATION_PATH/unifi.p12 -srcstoretype PKCS12 -srcstorepass aircontrolenterprise -alias unifi -noprompt
